...
public class Minesweeper extends JFrame {
    ...
    private SolverThreadWrapper thread = new SolverThreadWrapper(this);
    ...
    private Set<Cell> hintCells = new HashSet<>(); //
    ...
    private void loadUI() {
        ...
        gameTimer = new Timer(1000, e -> {
            if (currentGameTime < 100000) {
                timeLbl.setText("~ Time: " + currentGameTime);
            } else {
        ...
        Image img4 = Toolkit.getDefaultToolkit()
                .getImage("resources/bomb64x64.png");
        icons.add(img4);
    ...
    private void loadFileMenu() {
        menu.getAccessibleContext()
                .setAccessibleDescription("Playability settings.");
        menuBar.add(menu);
        helpMenu.getAccessibleContext()
                .setAccessibleDescription("Help/User Guide.");
        ...
        debugCb.addActionListener(e -> refresh());
     ...
     */
    private void clearNeighbours(int x, int y) {
        List<Cell> neighbours = getNeighbours(x, y); // Reuse code thats in solver class
        for (Cell c : neighbours) {
            // Only attempt to open closed cells
            if (c.isClosed() && !c.isFlagged()) {
    ...
    private void resetUIDetails() {
        ...
        setDetail("...");
        movesLbl.setText("Moves: " + moves);
        minesLbl.setText("~ Mines Left: " + minesLeft);
        enableAllBtns();
        gameTimer.start();
    }
     ...
     */
    private void openAllCells(String password) throws NoSuchAlgorithmException {
